...
def docker_copy(container_id: str, source_path: str, target_path: str) -> None:
    ...
    """
    print(f"Copying from container {container_id} at {source_path} to host at {target_path}")
...
def extract_timestamp_from_filename(filename: str) -> datetime:
    timestamp_str = '_'.join(filename.split("_")[:2]) # first two parts of the filename are expected to be date and time
    try:
...
def upload_incompletely_fetched_data(
    container_id: str, s3_prefix: str
):
    ...
    host_dir_path = f"{os.path.dirname(__file__)}/tmp/docker_container_task_data/{container_id}/"
    ...
    print(f"Compressed data to {compressed_path} ({size_bytes_human_readable(os.path.getsize(compressed_path))})")
    ...
    if not keys:
        ...
        print(f"No existing files found, using last modified timestamp string in S3 key: {timestamp_str}")
    else:
        ...
        existing_timestamp = extract_timestamp_from_filename(lexicographically_largest_file)
        ...
        if output_last_modified <= existing_timestamp:
            ...
            timestamp_str = (existing_timestamp + timedelta(seconds=1)).strftime("%Y-%m-%d_%H-%M-%S")
            print(f"Using adjusted timestamp string {timestamp_str} (largest existing + 1 second) for S3 upload to make it lexicographically larger than existing files (otherwise import won't trigger).")
        else:
            timestamp_str = output_last_modified.strftime("%Y-%m-%d_%H-%M-%S")
            print(f"Using last modified timestamp string for S3 key: {timestamp_str}")

    # Upload the compressed file to S3
    s3_key = f"{s3_prefix}/{timestamp_str}_{public_ip}_{flow_run_id}_incomplete.jsonl.zst"
